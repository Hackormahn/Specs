Pod::Spec.new do |s|
  s.name             = "PopcornTorrent"
  s.version          = "1.1.0.2"
  s.summary          = "Torrent client for iOS and tvOS (Used by PopcornTime)"
  s.homepage         = "https://github.com/PopcornTimeTV/PopcornTorrent"
  s.license          = 'MIT'
  s.author           = { "Popcorn" => "popcorn@time.tv" }
  s.tvos.source      = { :http => "https://github.com/PopcornTimeTV/PopcornTorrent/releases/download/1.1.0.2/PopcornTorrent.framework.zip" }
  s.ios.source       = { :http => "https://github.com/PopcornTimeTV/PopcornTorrent/releases/download/1.1.0.2/PopcornTorrent-iOS.framework.zip" }
  s.requires_arc = true
  s.deployment_target = '9.0'
  s.tvos.vendored_frameworks = "Carthage/Build/tvOS/PopcornTorrent.framework"
  s.ios.vendored_frameworks = "Carthage/Build/iOS/PopcornTorrent-iOS.framework"
end
